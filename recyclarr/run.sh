#!/bin/bash
echo "Starte Recyclarr Add-on V9.3..."
...
# TRaSH Profile Trash-IDs (fest, diese ändern sich nie)
SONARR_PROFILE_1080P_ID="9d142234e45d6143785ac55f5a9e8dc9"  # WEB-1080p
SONARR_PROFILE_2160P_ID="dfa5eaae7894077ad6449169b6eb03e0"  # WEB-2160p
RADARR_PROFILE_1080P_ID="d1d67249d3890e49bc12e275d989a7e9"  # HD Bluray + WEB
RADARR_PROFILE_4K_ID="64fb5f9858489bdac2af690e27c8f42f"     # UHD Bluray + WEB

# Hilfsfunktion: Custom Format ID dynamisch holen
get_id() {
    local SERVICE=$1
    local NAME=$2
    $RECYCLARR_BIN list custom-formats $SERVICE 2>/dev/null | grep -i "| $NAME " | awk '{print $2}'
}
...
R_DV=$(get_id radarr "Dolby Vision")
R_TRUEHD_ATMOS=$(get_id radarr "TrueHD ATMOS")
R_DDPLUS_ATMOS=$(get_id radarr "DDPlus ATMOS")
...
S_DV=$(get_id sonarr "Dolby Vision")
S_TRUEHD_ATMOS=$(get_id sonarr "TrueHD ATMOS")
S_DDPLUS_ATMOS=$(get_id sonarr "DDPlus ATMOS")
...
# Profile-Block mit Score generieren (referenziert per trash_id)
make_score_block() {
    local SCORE=$1
    shift
...
if [ "$SONARR_URL" != "" ] && [ "$SONARR_URL" != "null" ]; then
    ...
    # Aktive Sonarr Profile IDs sammeln
    SONARR_ACTIVE_PROFILES=()
    [ "$SONARR_1080P" = "true" ] && SONARR_ACTIVE_PROFILES+=("$SONARR_PROFILE_1080P_ID")
    [ "$SONARR_2160P" = "true" ] && SONARR_ACTIVE_PROFILES+=("$SONARR_PROFILE_2160P_ID")
...
EOF
    ...
    [ "$SONARR_1080P" = "true" ] && cat >> /data/recyclarr/recyclarr.yml << EOF
      - trash_id: $SONARR_PROFILE_1080P_ID  # WEB-1080p
        reset_unmatched_scores:
          enabled: true
EOF
    [ "$SONARR_2160P" = "true" ] && cat >> /data/recyclarr/recyclarr.yml << EOF
      - trash_id: $SONARR_PROFILE_2160P_ID  # WEB-2160p
        reset_unmatched_scores:
          enabled: true
EOF
    ...
    for ID_NAME in "$S_BRDISK:BR-DISK" "$S_LQ:LQ" "$S_LQ_TITLE:LQ (Release Title)" "$S_AV1:AV1" "$S_OBFUSCATED:Obfuscated" "$S_RETAGS:Retags" "$S_NO_RLSGROUP:No-RlsGroup"; do
    ...
    if [ "$ENABLE_HDR" = "true" ]; then
        ...
        write_cf_block /data/recyclarr/recyclarr.yml "$S_DV" "Dolby Vision" "$(make_score_block 600 "${SONARR_ACTIVE_PROFILES[@]}")"
    fi
    ...
    if [ "$ENABLE_ATMOS" = "true" ]; then
        ...
        write_cf_block /data/recyclarr/recyclarr.yml "$S_DDPLUS_ATMOS" "DDPlus ATMOS" "$(make_score_block 700 "${SONARR_ACTIVE_PROFILES[@]}")"
    fi
...
if [ "$RADARR_URL" != "" ] && [ "$RADARR_URL" != "null" ]; then
    ...
    # Aktive Radarr Profile IDs sammeln
    RADARR_ACTIVE_PROFILES=()
    [ "$RADARR_1080P" = "true" ] && RADARR_ACTIVE_PROFILES+=("$RADARR_PROFILE_1080P_ID")
    [ "$RADARR_4K" = "true" ] && RADARR_ACTIVE_PROFILES+=("$RADARR_PROFILE_4K_ID")
...
EOF
    ...
    [ "$RADARR_1080P" = "true" ] && cat >> /data/recyclarr/recyclarr.yml << EOF
      - trash_id: $RADARR_PROFILE_1080P_ID  # HD Bluray + WEB
        reset_unmatched_scores:
          enabled: true
EOF
    [ "$RADARR_4K" = "true" ] && cat >> /data/recyclarr/recyclarr.yml << EOF
      - trash_id: $RADARR_PROFILE_4K_ID  # UHD Bluray + WEB
        reset_unmatched_scores:
          enabled: true
EOF
    ...
    for ID_NAME in "$R_BRDISK:BR-DISK" "$R_LQ:LQ" "$R_LQ_TITLE:LQ (Release Title)" "$R_AV1:AV1" "$R_OBFUSCATED:Obfuscated" "$R_RETAGS:Retags" "$R_NO_RLSGROUP:No-RlsGroup"; do
        write_cf_block /data/recyclarr/recyclarr.yml "${ID_NAME%%:*}" "${ID_NAME#*:}" "$NEG"
    done

    if [ "$ENABLE_HDR" = "true" ]; then
        write_cf_block /data/recyclarr/recyclarr.yml "$R_HDR" "HDR" "$(make_score_block 500 "${RADARR_ACTIVE_PROFILES[@]}")"
        write_cf_block /data/recyclarr/recyclarr.yml "$R_DV" "Dolby Vision" "$(make_score_block 600 "${RADARR_ACTIVE_PROFILES[@]}")"
    fi

    if [ "$ENABLE_ATMOS" = "true" ]; then
        write_cf_block /data/recyclarr/recyclarr.yml "$R_TRUEHD_ATMOS" "TrueHD ATMOS" "$(make_score_block 750 "${RADARR_ACTIVE_PROFILES[@]}")"
        write_cf_block /data/recyclarr/recyclarr.yml "$R_DDPLUS_ATMOS" "DDPlus ATMOS" "$(make_score_block 700 "${RADARR_ACTIVE_PROFILES[@]}")"
    fi
